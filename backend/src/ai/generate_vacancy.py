...
class VacancyLLM(BaseModel):
    ...
    company: Annotated[Optional[str], StringConstraints(max_length=TEXT_MAX_LEN)]
...
def generate_vacancy(title: str, description: str) -> Optional[VacancyLLM]:
    ...
    prompt = (
        ...
        f"- Extract the company name in 'company' if possible (Limit it to {TEXT_MAX_LEN} characters).\n"
